...
PixelPos screen(Pos2D p) {
    ...
    int min_size = s.width_in_pixels < s.height_in_pixels ? s.width_in_pixels
                                                          : s.height_in_pixels;

    int x = p.x * (min_size / 2.) + s.width_in_pixels / 2.;
    int y = -p.y * (min_size / 2.) + s.height_in_pixels / 2.;

    return (PixelPos){x, y};
}

Pos3D translate(Pos3D p, Pos3D translation) {
    return (Pos3D){p.x + translation.x, p.y + translation.y,
                   p.z + translation.z};
}
